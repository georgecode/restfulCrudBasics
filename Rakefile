...
namespace :generate do
  ...
  desc "Create model, view, controller, migration, and spec :for NAME"
  task :mvc => [:model, :view, :controller, :migration, :spec]
...
#### VIEW ######### VIEW ######### VIEW ######### VIEW ######### VIEW #######N

desc "Create an view folder in app/models, e.g., rake generate:view NAME=User"
task :view do
  unless ENV.has_key?('NAME')
    raise "Must specificy view name, e.g., rake generate:view NAME=User"
  end

  view_name     = ENV['NAME'].camelize.downcase
  folder_path = APP_ROOT.join('app', 'views', view_name)
  Dir.mkdir(folder_path) unless File.exist?(folder_path)


  puts "Creating views"
  ['index','new', 'show', 'edit'].each do |file|
    view_path = APP_ROOT.join('app', 'views', view_name, (file +'.erb'))
    File.open(view_path, 'w+') do |f|
      f.write(<<-EOF.strip_heredoc)
          # hello from #{view_name} #{file} view
        end
      EOF
    end
  end
...
desc "Create an controller folder in app/models, e.g., rake generate:controller NAME=User"
task :controller do
  unless ENV.has_key?('NAME')
    raise "Must specificy controller name, e.g., rake generate:controller NAME=User"
  end

  controller_name     = ENV['NAME'].camelize.downcase
  ...
  if File.exist?(controller_path)
    raise "ERROR: View file '#{controller_path}' already exists"
  end
  ...
  File.open(controller_path, 'w+') do |f|
    f.write(<<-EOF.strip_heredoc)
      ...
      get '/#{controller_name}/edit' do

      end

      delete '/#{controller_name}/:id' do
...
#### MIGRATION #### MIGRATION #### MIGRATION #### MIGRATION #### MIGRATION ####
  ...
  task :migration do
    ...
    name     = ENV['NAME'].camelize.pluralize
    filename = “%s_%s_%s.rb” % [Time.now.strftime(‘%Y%m%d%H%M%S’),’create’, ENV[‘NAME’].pluralize]
    ...
    File.open(path, 'w+') do |f|
      f.write(<<-EOF.strip_heredoc)
        class Create#{name} < ActiveRecord::Migration
          def change
            create_table :#{name.pluralize.downcase} do |t|
            end
          end
        end
      EOF
